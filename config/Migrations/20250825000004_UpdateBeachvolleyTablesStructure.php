...
class UpdateBeachvolleyTablesStructure extends BaseMigration
{
    /**
     * Up Method.
     *
    ...
    {
        // Add missing columns to beachvolley_teams table
        $table = $this->table('beachvolley_teams');
        
        if (!$table->hasColumn('type_beachvolley')) {
            $table->addColumn('type_beachvolley', 'string', [
                'default' => '2x2',
                'limit' => 10,
                'null' => false,
                'after' => 'genre'
        ...
        if (!$table->hasColumn('beachvolley_category_id')) {
            $table->addColumn('beachvolley_category_id', 'integer', [
                'default' => null,
                'limit' => 11,
                'null' => true,
        ...
        if (!$table->hasColumn('beachvolley_district_id')) {
            $table->addColumn('beachvolley_district_id', 'integer', [
                'default' => null,
                'limit' => 11,
                'null' => true,
                'after' => 'beachvolley_category_id'
            ]);
        }
        
        if (!$table->hasColumn('beachvolley_organisation_id')) {
            $table->addColumn('beachvolley_organisation_id', 'integer', [
                'default' => null,
                'limit' => 11,
                'null' => true,
                'after' => 'beachvolley_district_id'
            ]);
        }
        ...
        if (!$table->hasColumn('responsable_nom_complet')) {
            $table->addColumn('responsable_nom_complet', 'string', [
                ...
                'limit' => 255,
                'null' => true,
                'after' => 'beachvolley_organisation_id'
            ]);
        }
    ...
    public function down(): void
    {
        ...
        $table->removeColumn('type_beachvolley');
        $table->removeColumn('reference_inscription');
        $table->removeColumn('beachvolley_category_id');
        $table->removeColumn('beachvolley_district_id');
        $table->removeColumn('beachvolley_organisation_id');

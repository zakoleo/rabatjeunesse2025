/**
 * Volleyball Team Registration Form Handler
 * Handles multi-step form wizard, validation, and player management
 */
document.addEventListener('DOMContentLoaded', function() {
    console.log('Initializing Volleyball Registration Form...');
    ...
    const PLAYER_REQUIREMENTS = {
        '6x6': { min: 6, max: 12, description: '6x6: 6-12 joueurs' },
        '4x4': { min: 4, max: 8, description: '4x4: 4-8 joueurs' }
    };
    ...
    async function loadAgeCategories() {
        try {
            const response = await fetch('/api/volleyball-date-ranges');
            if (!response.ok) {
                throw new Error('Failed to load age categories');
            }
            const data = await response.json();
            AGE_CATEGORIES = data.ageCategories || {};
            console.log('Loaded volleyball age categories:', AGE_CATEGORIES);
        } catch (error) {
    ...
    function validateStep1() {
        const requiredFields = [
            'nom_equipe',
            'volleyball_category_id',
            'genre',
            'type_volleyball',
            'volleyball_district_id',
            'volleyball_organisation_id',
            'adresse'
        ];
        ...
        const typeField = getFieldByName('type_volleyball');
    ...
    function validateStep3() {
        ...
        if (!requirements) {
            showNotification('Veuillez sélectionner un type de volleyball valide', 'error');
            return false;
        }
    ...
    // Volleyball type change
    const typeSelect = getFieldByName('type_volleyball');
    if (typeSelect) {
        typeSelect.addEventListener('change', function() {
            state.selectedType = this.value;
            const requirements = PLAYER_REQUIREMENTS[this.value];
            if (requirements) {
                const helpText = document.getElementById('type-volleyball-help');
                if (helpText) {
    ...
    const initialType = getFieldByName('type_volleyball');
    ...
    console.log('Volleyball Registration Form initialized successfully');
});
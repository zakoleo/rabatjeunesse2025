...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadAgeCategories() {
        const categorySelect = document.querySelector('[name="beachvolley_category_id"]');
        if (categorySelect) {
            // First, load basic info from select options
            const options = categorySelect.querySelectorAll('option');
            options.forEach(option => {
                if (option.value && option.textContent.trim()) {
                    beachvolleyCategories[option.value] = {
                        id: option.value,
    ...
    function fetchDetailedCategoryData() {
        fetch('/teams/getCategories?sport_id=5')
            ...
            .then(data => {
                ...
                data.categories.forEach(category => {
                    if (beachvolleyCategories[category.id]) {
                ...
                console.log('Updated beach volleyball categories with database data:', beachvolleyCategories);
            })
            .catch(error => {
    ...
    // Load beach volleyball types from server (if needed)
    function loadBeachVolleyTypes() {
        // Beach volleyball types are simpler, but we can load them dynamically if needed
        fetch('/teams/getSports')
            .then(response => response.json())
            .then(data => {
                console.log('Beach volleyball types loaded from server');
            })
            .catch(error => {
                console.warn('Using default beach volleyball limits');
            });
    }
        ...
        if (categorySelect) {
            ...
            options.forEach(option => {
                if (option.value && option.textContent.trim()) {
                    ...
                    if (ageRange && beachvolleyCategories[option.value]) {
                        beachvolleyCategories[option.value] = {
                            ...beachvolleyCategories[option.value],
                            ...ageRange
                        };
    ...
    function initializeWizard() {
        console.log('Initializing beach volleyball wizard...');
        ...
        // Set up beach volleyball type change handler for player limits
        const beachvolleyTypeSelect = document.querySelector('[name="type_beachvolley"]');
        if (beachvolleyTypeSelect) {
            beachvolleyTypeSelect.addEventListener('change', updatePlayerRequirements);
            // Initialize on load
    ...
    function updatePlayerRequirements() {
        const beachvolleyTypeSelect = document.querySelector('[name="type_beachvolley"]');
        if (!beachvolleyTypeSelect) return;
        
        const selectedType = beachvolleyTypeSelect.value;
        console.log('Beach volleyball type changed to:', selectedType);
    ...
    function updatePlayerCountDisplay(type, limits) {
        const infoElements = document.querySelectorAll('.beachvolley-info, .player-info');
    ...
    function getMinPlayers() {
        const beachvolleyTypeSelect = document.querySelector('[name="type_beachvolley"]');
        const selectedType = beachvolleyTypeSelect ? beachvolleyTypeSelect.value : '2x2';
        return playerLimits[selectedType] ? playerLimits[selectedType].min : 2;
    }
    
    function addPlayerForm() {
        const beachvolleyTypeSelect = document.querySelector('[name="type_beachvolley"]');
        const selectedType = beachvolleyTypeSelect ? beachvolleyTypeSelect.value : '2x2';
        const maxPlayers = playerLimits[selectedType] ? playerLimits[selectedType].max : 4;
    ...
    function getSelectedAgeCategory() {
        ...
        return beachvolleyCategories[categorySelect.value] || null;
    }
    ...
    function validatePlayerCount() {
        const beachvolleyTypeSelect = document.querySelector('[name="type_beachvolley"]');
        const selectedType = beachvolleyTypeSelect ? beachvolleyTypeSelect.value : '2x2';
        const limits = playerLimits[selectedType] || { min: 2, max: 4 };
    ...
    console.log('Beach volleyball wizard validation script loaded successfully');
});
 ...
 * Based on football-wizard-validation.js patterns and structure
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ===== ALL FUNCTIONS DEFINED AT START =====
    ...
    function isValidBirthDate(dateStr, field = null) {
        ...
        if (field && field.name && field.name.includes('joueurs[')) {
            ...
            if (category) {
                ...
                // No category selected - allow general basketball age range
    ...
    // DEFINE FUNCTIONS USING CONST TO AVOID HOISTING ISSUES
    
    // Validate field format only (for optional fields with values) - DEFINED FIRST
    const validateOptionalField = function(field) {
        console.log('validateOptionalField called with field:', field.name);
    ...
    // Validate individual field - calls validateOptionalField above
    ...
    function getSelectedAgeCategory() {
        ...
        console.log('  basketballCategories keys:', Object.keys(basketballCategories));
        console.log('  basketballCategories object:', basketballCategories);
        
        if (categorySelect && categorySelect.value) {
            const selectedId = categorySelect.value;
            const category = basketballCategories[selectedId];
            
            console.log(`  Looking for category ID: ${selectedId}`);
            console.log(`  Found category:`, category);
            
            if (!category) {
                console.error(`❌ Category ID ${selectedId} not found in basketballCategories object!`);
                console.error(`Available category IDs:`, Object.keys(basketballCategories));
            }
    ...
    // Get selected basketball type
    function getSelectedBasketballType() {
        const typeField = document.querySelector('[name="type_basketball"]');
        ...
        console.warn('No basketball type selected');
        return null;
    }
    ...
    function validatePlayerCount() {
        ...
        const basketballType = getSelectedBasketballType();
        const limits = playerLimits[basketballType];
        
        if (!limits) {
            console.error('No limits found for basketball type:', basketballType, 'Available:', Object.keys(playerLimits));
            const container = document.getElementById('joueursContainer');
            if (container) {
                showContainerError(container, `Erreur: Type de basketball "${basketballType}" non reconnu. Veuillez sélectionner un autre type.`);
            }
            return false;
        }
        
        console.log(`Players: ${playersCount}, Required: ${limits.min}-${limits.max} for ${basketballType}`);
        
        if (playersCount < limits.min) {
            const container = document.getElementById('joueursContainer');
            if (container) {
                showContainerError(container, `Vous devez ajouter au moins ${limits.min} joueurs pour le ${basketballType}`);
            }
            return false;
        }
    ...
    function fetchDetailedCategoryData() {
        // Try basketball-specific endpoint first, fallback to generic
        const categoriesUrl = (window.API_URLS && window.API_URLS.getBasketballCategories) 
            ? window.API_URLS.getBasketballCategories
            : (window.APP_BASE_URL || '') + 'teams/getBasketballCategories';
        
        console.log('🔍 Fetching basketball categories from URL:', categoriesUrl);
        console.log('🔍 Current categoriesLoaded state:', categoriesLoaded);
        console.log('🔍 Current basketballCategories:', basketballCategories);
        ...
        fetch(categoriesUrl)
            ...
            .then(data => {
                ...
                if (data.categories && Array.isArray(data.categories)) {
                    ...
                    console.log('Basketball categories loaded from database:', basketballCategories);
    ...
    function loadBasketballTypes() {
        const basketballTypesUrl = (window.API_URLS && window.API_URLS.getBasketballTypes) 
            ? window.API_URLS.getBasketballTypes
            : (window.APP_BASE_URL || '') + 'teams/getBasketballTypes';
        
        console.log('Fetching basketball types from URL:', basketballTypesUrl);
        fetch(basketballTypesUrl)
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP ${response.status}: ${response.statusText}`);
                }
            ...
            .then(data => {
                console.log('Received basketball types data:', data);
                
                if (data.basketball_types && Array.isArray(data.basketball_types)) {
                    // Clear any existing limits and populate from database
                    playerLimits = {};
                    data.basketball_types.forEach(type => {
                    ...
                    console.error('Invalid basketball types data format:', data);
                    throw new Error('Invalid data format received');
                }
            })
            .catch(error => {
                console.error('Failed to load basketball types data:', error);
                playerLimitsLoaded = false;
                
                // Show error message to user
                showGlobalError('Erreur de connexion: Impossible de charger les types de basketball. Veuillez rafraîchir la page ou contacter l\'administrateur.');
    ...
    function setupPlayerManagement() {
        ...
        // Update requirements when basketball type changes
        const typeField = document.querySelector('[name="type_basketball"]');
        if (typeField) {
        ...
        const categoryField = document.querySelector('[name="basketball_category_id"]');
        if (categoryField) {
    ...
    function addPlayer() {
        ...
        if (limits && currentPlayers >= limits.max) {
            showContainerError(container, `Maximum ${limits.max} joueurs pour le ${basketballType}`);
            return;
        }
    ...
    function generateMinimumPlayers() {
        ...
        if (!limits) {
            console.log('No limits found for basketball type:', basketballType);
            return;
        }
        
        console.log(`Generating minimum players for ${basketballType}: need ${limits.min}, have ${currentPlayerCount}`);
    ...
    function updatePlayerRequirements() {
        ...
        if (!basketballType) {
            if (requirementsElement) {
                requirementsElement.innerHTML = `
                    <strong>⚠️ Sélectionnez un type de basketball</strong><br>
        ...
        if (limits && requirementsElement) {
            ...
            requirementsElement.innerHTML = `
                <strong>Composition de l'équipe ${basketballType}</strong><br>
            ...
            requirementsElement.innerHTML = `
                ...
                Le type "${basketballType}" n'est pas reconnu. Veuillez rafraîchir la page.
            `;
        }
    ...
    window.removePlayer = function(button) {
        ...
        if (limits && currentCount <= limits.min) {
            console.log(`Cannot remove player: minimum ${limits.min} required for ${basketballType}`);
            
            // Show temporary message
            const message = document.createElement('div');
            message.className = 'alert alert-warning';
            message.style.marginTop = '10px';
            message.textContent = `Impossible de supprimer: minimum ${limits.min} joueurs requis pour le ${basketballType}`;
    ...
    console.log('Basketball wizard validation setup complete');
});console.log('Basketball validation loaded at: ' + new Date().toISOString());

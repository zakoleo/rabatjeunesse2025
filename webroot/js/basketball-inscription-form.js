/**
 * Basketball Team Registration Form Handler
 * Handles multi-step form wizard, validation, and player management
 */
document.addEventListener('DOMContentLoaded', function() {
    console.log('Initializing Basketball Registration Form...');
    ...
    const PLAYER_REQUIREMENTS = {
        '3x3': { min: 3, max: 4, description: '3x3: 3-4 joueurs' },
    ...
    async function loadAgeCategories() {
        try {
            const response = await fetch('/api/basketball-date-ranges');
            if (!response.ok) {
                throw new Error('Failed to load age categories');
            }
            const data = await response.json();
            AGE_CATEGORIES = data.ageCategories || {};
            console.log('Loaded basketball age categories:', AGE_CATEGORIES);
        } catch (error) {
    ...
    function validateStep1(stepContainer) {
        ...
        // Get form fields
        const categoryField = getFieldByName('basketball_category_id');
        const typeField = getFieldByName('type_basketball');
        const districtField = getFieldByName('basketball_district_id');
        const organisationField = getFieldByName('basketball_organisation_id');
        ...
        if (!typeField || !typeField.value) {
            showNotification('Veuillez sélectionner un type de basketball', 'error');
    ...
    function validateStep3(stepContainer) {
        ...
        if (!requirements) {
            showNotification('Type de basketball non valide', 'error');
            return false;
        }
    ...
    function ajouterJoueur() {
        ...
        if (!state.selectedType) {
            showNotification('Veuillez d\'abord sélectionner le type de basketball', 'error');
            return;
        }
    ...
    function initialize() {
        ...
        const initialTypeField = getFieldByName('type_basketball');
        ...
        console.log('Basketball registration form initialized successfully');

...
<div class="admin-concours-index">
    <!-- Stats Cards -->
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">Répartition par type de concours</h5>
                </div>
                <div class="card-body">
                    <div class="row">
            ...
            <?= $this->Form->create(null, ['type' => 'get', 'class' => 'form-inline']) ?>
                ...
                <div class="form-group mr-3">
                    <?= $this->Form->control('type', [
                        'class' => 'form-control',
                        'options' => ['' => 'Tous les types'] + $concoursTypes,
                ...
                <?= $this->Html->link('Réinitialiser', ['action' => 'concours'], ['class' => 'btn btn-secondary ml-2']) ?>
            <?= $this->Form->end() ?>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Participants Concours</h3>
        </div>
        <div class="card-body">
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge badge-primary"><?= h($participant->type_concours) ?></span>
                            </td>
                            <td>
                                <?php if ($participant->has('concours_category')): ?>
                                    <span class="badge badge-info">
                                        <?= h($participant->concours_category->gender) ?> - 
                                        <?= h($participant->concours_category->age_category) ?>
                                    </span>
                                <?php endif; ?>
                            </td>
                            ...
                            <td>
                                <div class="btn-group" role="group">
                                    <?= $this->Html->link('<i class="fas fa-eye"></i>', 
                                        ['action' => 'viewConcoursFullParticipant', $participant->id], 
...
function updateStatus(id, status) {
    if (confirm('Êtes-vous sûr de vouloir changer le statut?')) {
        $.ajax({
            url: '<?= $this->Url->build(['action' => 'updateConcoursStatus']) ?>',
            method: 'POST',
            data: {
                id: id,
                status: status
...
function viewParticipant(id) {
    $.ajax({
        url: '<?= $this->Url->build(['action' => 'viewConcoursParticipant']) ?>/' + id,
        method: 'GET',
        success: function(response) {

 ...
 */
?>
<div class="volleyball-types content">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>🏐 Volleyball Types</h2>
        <?= $this->Html->link('🏐 Dashboard', ['action' => 'index'], ['class' => 'btn btn-outline-secondary']) ?>
    </div>
    
    <div class="card shadow">
        <div class="card-header"><h6 class="m-0 font-weight-bold text-info">Types de terrain Volleyball</h6></div>
        <div class="card-body">
            <?php if (!empty($types)): ?>
                    ...
                    <tbody>
                        <?php foreach ($types as $type): ?>
                            <tr>
                                ...
                                <td><span class="badge badge-light"><?= count($type->volleyball_categories ?? []) ?> catégories</span></td>
            ...
            <?php else: ?>
                <p class="text-center text-muted">Aucun type volleyball configuré</p>
            <?php endif; ?>
        </div>
    </div>
</div>